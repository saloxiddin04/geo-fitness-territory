...
// Faol sessiyani tiklash (app restart bo'lganda)
export const fetchActiveSession = createAsyncThunk(
  'running/fetchActive',
  async (_, { rejectWithValue }) => {
    try {
      const response = await api.get('/running/active');
...
export const startSession = createAsyncThunk(
  ...
  async ({ latitude, longitude }, { rejectWithValue }) => {
    try {
      const response = await api.post('/running/start', { lat: latitude, lng: longitude });
      return response.data.data;
    } catch (error) {
...
const runningSlice = createSlice({
  ...
  extraReducers: builder => {
    ...
    // Faol sessiyani tiklash
    builder
      .addCase(fetchActiveSession.fulfilled, (state, action) => {
        if (action.payload?.session) {
          state.activeSession = action.payload.session;
          state.isRunning = true;
          state.startTime = new Date(action.payload.session.startedAt).getTime();
        }
      });
  },
});
